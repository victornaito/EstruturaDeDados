...
int main()
{
    ...
    printf("Insira o indice do elemento a ser encontrado l c \n");
    scanf("%d %d", &buscal, &buscac);
    if (buscal > n-1 || buscac > m-1)           /*Feita a validação de casos de teste*/
    {
        printf("Este indice é maior que o tamanho da matriz\n");  /* o valor de indice colocado é maior que o  tamanho da matriz*/
    }
    else{
        busindice = buscaPeloIndice(raiz, buscal, buscac);
        if (busindice != NULL) printf("%d\n", busindice->info);  /*se a celula existir imprime o valor*/
        else printf("NULL\n");          /*se não existir a celula retorna NULL*/
    }
...
Celula *buscaPeloIndice (Celula *raiz, int l , int c)
{
    ...
    percorreLinha = percorreLinha->e; /*dou o primeiro passo na linha*/
    while(percorreLinha->c != -1){
        if(percorreLinha->c == c){
            return percorreLinha;

        }
        else if(percorreLinha->c > c)
            return NULL; 
    }
    return NULL;
}
